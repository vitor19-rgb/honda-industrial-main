...
function inicializarDadosEngenharia() {
    ...
    // Criar uma OS de aprovação falsa para teste, caso não exista
...
// 4. PADRONIZAÇÃO TÉCNICA (Planos de Manutenção)
...
// 5. CAIXA DE APROVAÇÕES DE OS CRÍTICAS
...
function reprovarOS(id) {
    ...
    let os = osLista.find(o => o.id === id);
    if(os) {
        os.status = 'Em Andamento'; // Devolve ao técnico
        localStorage.setItem('honda_os_lista', JSON.stringify(osLista));
